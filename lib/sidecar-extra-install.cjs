...
const SCRIPT_STEM = {
  ...
  "stems-melband": "install-sidecar-stems-melband",
...
function installSidecarExtra(extraId) {
  ...
  // Large pip extras (generate / vocal-rvc) can take a while; never hang forever.
  ...
  const r =
    script.kind === "ps1"
      ? spawnSync(
          "powershell",
          ["-NoProfile", "-ExecutionPolicy", "Bypass", "-File", script.file],
          { cwd: ROOT, encoding: "utf8", maxBuffer: 8 * 1024 * 1024, timeout: INSTALL_TIMEOUT_MS },
        )
      : spawnSync("bash", [script.file], {
          cwd: ROOT,
          encoding: "utf8",
  ...
  if (r.error) {
    const timedOut = r.error.code === "ETIMEDOUT" || /ETIMEDOUT/i.test(r.error.message || "");
    return {
      ok: false,
      extraId: id,
      mode: timedOut ? "install-timeout" : "install-failed",
      error: timedOut
        ? `Install timed out after ${INSTALL_TIMEOUT_MS / 60_000} minutes — try \`${hint}\` in a terminal`
        : r.error.message || "Install failed to start",
      installHint: hint,
    };
  }
...
function probeSidecarExtraInstallEnv() {
  if (!fs.existsSync(path.join(SIDECAR, "ai_sidecar", "main.py"))) {
    return {
      mode: "bundled-readonly",
      writable: false,
  ...
  return {
    mode: "writable",
    writable: true,
    message: "Local ai-sidecar/.venv found — Install can run pip extras.",
  };
}

module.exports = { installSidecarExtra, SCRIPT_STEM, softRestartSidecar, probeSidecarExtraInstallEnv };
