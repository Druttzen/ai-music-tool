/** Shared Canvas handoff paths for Tauri Studio (app-data colocated). */
const fs = require("fs");
...
/**
 * Suite root is `{STUDIO_DATA_DIR}` when set (Studio runtime), else a temp fixture
 * for Node tests — never Documents/AI Suite for writes.
 */
function suiteDir() {
  const studioData = (process.env.STUDIO_DATA_DIR || "").trim();
  if (studioData) {
    return path.resolve(studioData);
  }
  return path.join(os.tmpdir(), "ai-music-creator-studio-suite");
}
...
function expandPathTemplate(template) {
  ...
  const studioData = env.STUDIO_DATA_DIR || "";
  ...
  return template
    ...
    .replace(/\$STUDIO_DATA/g, studioData)
...
function canvasReleasesFallbackUrl() {
  return (
    config.canvas?.releasesUrl ||
    config.canvas?.installUrl ||
    config.canvas?.repoUrl ||
    "https://github.com/Druttzen/ai-canvas-tool/releases"
  );
}
