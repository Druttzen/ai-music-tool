...
function installationCandidatesForAddon(addonId) {
  const list = addonMeta(addonId)?.installCandidates?.[canvasPlatformKey()] || [];
  return list.map(expandPathTemplate).filter(Boolean);
}

function findAddonInstallation(addonId) {
  return installationCandidatesForAddon(addonId).find((p) => p && fs.existsSync(p)) || null;
}
...
module.exports = {
  ...
  installationCandidatesForAddon,
  canvasAddonMeta,
