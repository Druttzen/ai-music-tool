...
function launchSuiteAddon(addonId) {
  const id = String(addonId || "canvas");
...
/**
 * Write music-video handoff JSON (+ optional copied assets) under AI Suite exports.
 * @param {{ prompt?: string, bpm?: string, idea?: string, audioPath?: string|null, coverPath?: string|null }} payload
 */
async function exportMusicVideoHandoff(payload = {}) {
  const dir = musicVideoExportsDir();
  await fs.promises.mkdir(dir, { recursive: true });
  const handoffPath = musicVideoHandoffFile();
  const handoff = {
    version: 1,
  ...
  return {
    ...
    path: handoffPath,
    exportsDir: dir,
    message: `Music video handoff written to ${dir} — import this folder in Glitchframe`,
  };
}
