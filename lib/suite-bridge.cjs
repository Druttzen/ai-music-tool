...
async function openCanvasInstallDocs({ mode = "docs", message = "", preferReleases = false } = {}) {
  const url = preferReleases ? canvasReleasesFallbackUrl() : canvasInstallFallbackUrl();
...
async function installCanvasAddon() {
  ...
  if (localInstaller) {
    try {
      ...
      return {
        ...
        path: localInstaller,
        error: error instanceof Error ? error.message : "Could not open local Canvas installer",
      };
    }
  }
  ...
  try {
    const release = await httpsGetJson(
      `https://api.github.com/repos/${meta.githubOwner}/${meta.githubRepo}/releases/latest`,
    );
    ...
    try {
      ...
      return {
        ...
        mode: "downloaded",
        path: destination,
        error: error instanceof Error ? error.message : "Downloaded installer but could not open it",
      };
    }
  } catch (error) {
    const msg = String(error?.message || error || "download failed");
    // Only the releases API 404 means "no release" — not asset download 404s.
    const noRelease = /^GitHub API 404\b/.test(msg);
    if (noRelease) {
      return openCanvasInstallDocs({
        mode: "no-release",
        message: "Could not find a latest Canvas release — opened releases page",
        preferReleases: true,
      });
    }
    return {
