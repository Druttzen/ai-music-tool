...
/**
 * Install / download Canvas addon: local installer → GitHub release → install URL.
 */
async function installCanvasAddon() {
  ...
  if (owner && repo) {
    try {
      ...
      // fall through to browser install page
    }
  }

  const installUrl = meta?.installUrl || meta?.repoUrl;
  if (installUrl) {
    await openExternalSafe(installUrl);
    return { ok: true, mode: "browser", url: installUrl };
  }

  return { ok: false, error: "No Canvas install source configured" };
}
