...
- **Canvas install follow-ups** — API 404 vs download failures separated; Setup.exe matching hardened; Addons copy/docs aligned.

## Highlights (v0.50.9)

- **Addons + Canvas install** — left **Addons** panel installs Canvas and sidecar extras into the Studio app `data/` folder (`profile`, `addons`, `sidecar`, `tools`). Browser copies the npm command instead.
...
- **Canvas handoff P2** — sanitize Electron artwork extension; `launched` is false on folder-only fallback when Canvas Tool is not started.
- **CC0 catalog sync** — regen awesome-suno catalog to clear sync CI drift.
...
- **Fail-safe bug found dialog** — blocking popup when critical issues are detected, with live fix progress and a gated **Finished** button.
- **In-app fix & push** — maintainer mode runs auto-fix, commit, and push from the dialog (local or cloud dispatch).
- **CI reliability** — sidecar `httpx` dependency and TypeScript fix for fail-safe bridge; e2e no longer blocked by the dialog.
...
- **Section DAW** — mood words split correctly for metatag delivery tags (fixes runtime crash on scaffold insert).
- **Vocal Embed hydration** — voice style from character session applies after mount (no SSR/client mismatch).
- **Electron releases** — consistent installer filename via `artifactName` (`AI-Music-Creator-Setup-{version}.exe`).
- **E2e** — Section DAW mood metatags + Suno 5.5 Pro tools (My Taste, Album mode).

## Highlights (v0.47.2)

- **Diagnostic fixes** — Section DAW metatags use mood words; Vocal Embed picks Voice Character Studio compact lines; major-version upgrade clears voice/Maestro/vocal-embed session instead of rehydrating stale autosave.
- **Analyzer hardening** — busy guards on audio/image analysis; safer blob fetch (`res.ok`); MusicGen reports always carry `analysisEngine`.
- **Maestro / Co-Producer** — LLM brief and patches aligned on `selectedGenres` (max 3); stale snapshot ref during long LLM calls; Co-Producer and AudD failures surface toasts.
- **Persistence** — legacy `genres` field migrates to `selectedGenres` on load/import.
...
npm run sidecar:cuda        # CUDA torch when NVIDIA GPU present (auto after torch extras)
