...
import { SUPPORTED_AUDIO_ACCEPT } from "../lib/analyzer-file-types";
import { formatVocalEmbedTime } from "../lib/vocal-embed-engine";
import { useVocalEmbedStudio } from "../hooks/use-vocal-embed-studio";
import { Panel } from "./ui-blocks";

export const CenterVocalEmbedStudio = memo(function CenterVocalEmbedStudio() {
  const studio = useVocalEmbedStudio();
  const {
    generatedLyrics,
    audioAnalysis,
    plan,
          ...
          <div className="mt-2">
            <input
              ...
              onChange={(e) => {
                ...
                if (!file) return;
                if (!handleGuideVocalFileChange(file)) e.target.value = "";
              }}
            />
            <button
              type="button"
