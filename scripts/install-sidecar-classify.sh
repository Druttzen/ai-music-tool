...
echo "Installing classify extra (torch + transformers)…"
"$VENV/bin/pip" install -e "$SIDECAR[classify]"
echo "Classify extra installed. Restart the sidecar: npm run sidecar"
